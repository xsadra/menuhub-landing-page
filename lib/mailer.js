/**
 * Admin notification dispatcher (Resend / SMTP fallback)
 *
 * @author Sadra Babai
...
try {
  ...
  // Optional dependency if running purely on standard SMTP
}
...
// Dispatch helper: checks Resend first, then custom SMTP, falls back to console in local dev
async function dispatchEmail({ to, subject, html, text }) {
  ...
  if (resendApiKey && ResendClient) {
    try {
      ...
      console.warn('[mailer:resend] Send failure, checking SMTP fallback:', err.message);
    }
  }
  ...
  if (smtpHost && process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
      ...
      console.error('[mailer:smtp] Send failure:', err.message);
    }
  }

  // Dev fallback: log output when no credentials are configured
  console.log(`[mailer:dev] Mock dispatch to ${to}: "${subject}"`);
  return { success: true, provider: 'dev_mock' };
}
